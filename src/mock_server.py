...
async def _start_simulate():
    ...
    p = multiprocessing.Process(target=start_simulate, kwargs={"pipe": child_pipe, "model": model})  # 使用接收到的参数创建新进程
    p.start()  # 启动新进程
        ...
        while True:
            if parent_pipe.poll():  # 检查管道是否有新的数据
                ...
                if message == "END":
                    ...
                    break
                with open(f'{os.getenv("ROOT_PATH")}/static/simulate_logs/pipe_only_president_{model}.txt', "a") as f:
