...
def upload_file_to_oss(file_path: str, content: str | bytes) -> str:
    ...
    log.info(f"upload file to oss: file_path:{file_path} request_id:{result.request_id}")
    if result.status == 200:
        return f"https://{os.getenv('OSS_BUCKET_NAME')}.{os.getenv('OSS_ENDPOINT')}/{file_path}"
    else:
        log.error(f"upload file to oss failed: file_path:{file_path} status:{result.status}")
        return ""
...
def download_file_from_oss(file_path: str) -> bytes:
    ...
    log.info(f"download file from oss: file_path:{file_path} request_id:{result_stream.request_id}")
    ...
    if result_stream.status == 200:
        ...
        log.error(f"download file from oss failed: file_path:{file_path} status:{result_stream.status}")
        return bytes(0)
...
def migrate_img_oss(file_url: str, prompt: str) -> str:
    ...
    if res.status_code == 200:
        log.info(f"migrate to oss: url:{file_url} prompt:{prompt} status:{res.status_code} res:{res.json()}")
        return res.json()["url"]
    else:
        log.error(f"migrate to oss failed: url:{file_url} prompt:{prompt} status:{res.status_code}")
        return ""
