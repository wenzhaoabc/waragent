...
def p_generate_cypher_prompt() -> str:
    return (
        "You are an expert of neo4j database and cypher script."
        "Your task is to convert questions about contents in a Neo4j database to Cypher queries to query the Neo4j database."
        "Please Use only the provided relationship types and properties. Do not use any other relationship types or properties that are not provided."
    )


def p_get_fewshot_examples(openai_api_key: str, base_url: str):
    """生成Cypher查询所用的example"""
    return f"""
